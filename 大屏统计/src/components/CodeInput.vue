        ...
        <div class="code-input">
            ...
            <template v-for="(v, index) in values" :key="index">
                <input
                    ...
                    type="number" pattern="[0-9]" :style="{
                        width: `${props.fieldWidth}px`,
                        height: `${props.fieldHeight}px`,
                    }" :autoFocus="autoFocus && index === autoFocusIndex" :data-id="index" :value="v" :ref="
    (el) => {
        if (el) inputs[index + 1] = el;
    }
" v-on:input="onValueChange" v-on:focus="onFocus" v-on:keydown="onKeyDown" :required="props.required"
                    :disabled="props.disabled" maxlength="1" />
            </template>
        </div>
    </div>
</template>
