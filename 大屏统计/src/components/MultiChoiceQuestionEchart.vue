<template>
	<div>
		...
		<el-radio-group v-model="chartType" label="type control" style="margin: 20px auto;" class="buttonShadow">
...
<script>
	...
	import MultiChoiceQuestionEchartVue from './MultiChoiceQuestionEchart.vue';
	export default {
		name: MultiChoiceQuestionEchartVue,
		props: {
			answersGet: Object,
			optionsGet: Object,
			indexGet: Number,
		...
		data() {
			return {
				...
				options: this.optionsGet.extensions.checkbox.options,
				optionPieData: [],
				optionBarData: [
					[],
					[]
				], //一个x数据一个data(y就没给了，直接value),接value),不过Line和Bar共用一套
				chartType: 'pie',
		...
		methods: {
			...
			getAnswerPercent: function(dataItem) {
				var percent = Math.round((dataItem.count / this.optionCountSum) * 100);
				return percent;
			},
			printTest: function() {
				console.log("answersGet:", this.answersGet);
				console.log("optionsGet:", this.optionsGet);
				console.log("options", this.options);
				console.log("answers", this.answers.answers);
				console.log("统计信息：", this.matchData());
			},
			lineInit: function() {
